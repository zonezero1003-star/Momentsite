...
export function marketplaceRouter({ backendWallet }) {
  ...
  /**
   * POST /api/marketplace/build-buy-tx
   * body: { assetAddress, buyerPublicKey }
   * Builds (but does not send) a plain SOL transfer from buyer -> seller
   * for the listing's price. Built server-side so the RPC endpoint (which
   * may be a keyed provider, see SOLANA_RPC_URL) never has to be exposed
   * to the browser just to fetch a blockhash. Buyer signs and pays the
   * network fee themselves via Phantom — same pattern as minting.
   */
  router.post("/build-buy-tx", async (req, res) => {
    try {
      const { assetAddress, buyerPublicKey } = req.body;
      if (!assetAddress || !buyerPublicKey) {
        return res.status(400).json({ error: "assetAddress and buyerPublicKey are required" });
      }
      ...
      const lamports = Math.round(listing.priceSol * 1_000_000_000);
      const { blockhash } = await connection.getLatestBlockhash();

      const transaction = new Transaction({
        feePayer: new PublicKey(buyerPublicKey),
        recentBlockhash: blockhash,
      }).add(
        SystemProgram.transfer({
          fromPubkey: new PublicKey(buyerPublicKey),
          toPubkey: new PublicKey(listing.sellerPublicKey),
          lamports,
        })
      );
      ...
      console.error("build-buy-tx failed:", err);
      res.status(500).json({ error: err.message });
    }
  });
