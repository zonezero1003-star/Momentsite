...
import { getUmi, uploadMetadata, buildMintTransaction } from "../services/metaplex.js";
...
export function predictionsRouter({ backendWallet, latestOdds }) {
   ...
   * Generates a real photoreal PNG (portrait, via Gemini) WITHOUT minting
   * or reserving anything — pure preview. /build regenerates its own PNG
   * at mint time rather than reusing this one, so the preview and the
   * minted asset won't be byte-identical, but both are always PNG.
   */
  router.post("/preview", async (req, res) => {
    try {
      const { fixtureId, match, predictedEvent, predictedWindow, predictedParticipant, backingTeamName } = req.body;
      if (!fixtureId || !predictedEvent) {
        return res.status(400).json({ error: "fixtureId and predictedEvent are required" });
      }
      ...
      const { imageUri } = await generateUnresolvedImage(umi, {
        match: match || fixtureId, predictedEvent, predictedWindow, oddsAtPrediction,
        editionNumber, editionCap: config.editionCap, backingTeamName,
      });
  ...
  router.post("/build", async (req, res) => {
    try {
      const { ownerPublicKey, fixtureId, match, predictedEvent, predictedWindow, predictedParticipant, backingTeamName } = req.body;
      ...
      // /build always regenerates its own image via Gemini rather than
      // trusting a client-supplied imageUri, so the asset that actually
      // gets minted is guaranteed to be a fresh, wallet-safe PNG.
      const { imageUri, source: imageSource } = await generateUnresolvedImage(umi, {
        match: match || fixtureId, predictedEvent, predictedWindow, oddsAtPrediction,
        editionNumber, editionCap: config.editionCap, backingTeamName,
      });
      ...
      const metadata = {
        ...
        attributes: [
          ...
          ...(backingTeamName ? [{ trait_type: "Backing", value: backingTeamName }] : []),
      ...
      pendingMintDetails.set(assetAddress, {
        ...
        predictedParticipant, backingTeamName, oddsAtPrediction, editionNumber, imageUri, imageSource,
      });
  ...
  router.post("/confirm", async (req, res) => {
    try {
      ...
      // getTransaction can trail actual confirmation by a second or two —
      // "confirmed" means the block landed, not that THIS RPC node has
      // indexed it yet. A single immediate lookup is prone to false
      // "not found" errors right after the wallet's signature comes back,
      // so poll a few times before actually giving up.
      const txInfo = await getTransactionWithRetry(signature);
      ...
      const record = await PredictionsStore.create({
        ...
        predictedParticipant: details.predictedParticipant,
        backingTeamName: details.backingTeamName,
...
/**
 * Polls getTransaction a few times before giving up. "confirmed" commitment
 * means the block landed, not that whichever RPC node answers this
 * particular call has indexed the transaction yet — especially on a
 * shared/free-tier or rate-limited RPC, that indexing can trail actual
 * confirmation by a second or more. A single immediate lookup right after
 * the wallet hands back a signature is exactly the situation that
 * produces false "transaction not found" errors.
 */
async function getTransactionWithRetry(signature, { attempts = 5, delayMs = 1500 } = {}) {
