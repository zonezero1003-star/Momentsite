/**
 * Gemini is OPT-IN and disabled by default (GEMINI_ENABLED must be set to
 * "true"). With it off, every card is the deterministic template from
 * cardImage.js — rasterized to PNG via sharp, portrait, with embedded
 * fonts so it renders correctly on any host. When/if Gemini is turned
 * back on, it's tried first for a photoreal PNG; if it fails to generate
 * OR fails to upload (network blip, quota, refusal, timeout), this falls
 * back to the same deterministic PNG template rather than erroring out.
 *
...
export async function generateUnresolvedImage(umi, params) {
  if (!GEMINI_ENABLED) {
    ...
    return { imageUri: uri, source: "template" };
  }
