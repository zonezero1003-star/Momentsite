 ...
 *
 * Deliberately NOT using umi.uploader.uploadJson() here — it's been
 * observed (repeatedly, in production logs) returning a garbage/
 * truncated URI (e.g. "https://arweave.net/h", a single character)
 * while umi.uploader.upload() with a manually-built file has never once
 * shown that failure mode. This matches a known upstream issue:
 * @metaplex-foundation/umi-uploader-irys@0.9.x depends on the deprecated
 * Solana RPC method getRecentBlockhash (removed by Solana in the RPC
 * v2.0 deprecation cleanup — see github.com/metaplex-foundation/umi/
 * issues/144, "File upload does not work anymore whatever the rpc
 * used"), and the fix for the related arweave-vs-irys-gateway host issue
 * (github.com/metaplex-foundation/umi PR #148) only shipped in the 1.x
 * line — this project is pinned to ^0.9.2, which cannot receive it via
 * a normal `npm install` since 0.x versions don't cross the minor-
 * version boundary under a caret range. Upgrading to 1.x is the more
 * complete fix but requires a breaking import change
 * (`umi-uploader-irys/node` instead of the bare import) across a few
 * tightly-coupled packages — do that as a deliberate, tested upgrade
 * later rather than blind here. This workaround needs no dependency or
 ...
 * PNG is the only image format Moment NFTs use — most wallets (Phantom
 * included) don't render image/svg+xml at all, so there is no SVG path
 * here or anywhere else in the mint/resolve flow.
 */
