/**
 * Deterministic fallback card — used ONLY when Gemini fails to generate
 * or upload an image. Every value on the card comes directly from
 * TxLINE's data (or the prediction record derived from it), so it's a
 * safe, always-available fallback with no external API calls of its own.
 *
 * IMPORTANT: this module builds an SVG string internally, but that SVG
 * NEVER leaves this module or gets uploaded anywhere. `buildUnresolvedCardPng`
 * and `buildResolvedCardPng` below rasterize it to a real PNG buffer
 * before returning it — most wallets (Phantom included) don't render
 * image/svg+xml at all, so the fallback has to be a real raster image to
 * be "functional at the mint level" the same way the Gemini path is.
 * Nothing outside this file should ever see or upload the SVG itself;
 * only buildUnresolvedCardPng/buildResolvedCardPng are meant to be
 * called by other modules.
 *
 * RASTERIZER: @resvg/resvg-js, NOT sharp. sharp's SVG backend (librsvg)
 * has a long-standing, widely-reported bug where embedded @font-face
 * fonts silently fail to render — text comes out as missing-glyph boxes
 * or vanishes entirely, especially on cloud hosts (see e.g.
 * github.com/lovell/sharp/issues/2499, #2458, #1722, #2399). resvg-js
 * (Rust-based, precompiled, no system deps) takes font files directly
 * via its `font.fontFiles` option instead of parsing @font-face CSS, so
 * it isn't subject to that bug and doesn't depend on anything installed
 * on the host (no fontconfig, no font packages, works identically on
 * Railway/Render/Fly/Coolify/anywhere).
 *
...
// Passed straight to resvg-js as file paths — it reads and indexes these
// itself. loadSystemFonts: false means it never touches (or needs) the
// host's fontconfig at all, so behavior is identical on every platform.
// Family names below (SERIF_STACK/MONO_STACK) MUST match the actual
// name-table family names baked into these .ttf files ("IBM Plex Serif"
// / "IBM Plex Mono") — confirmed via fontTools, not guessed.
const CARD_FONT_FILES = [
  path.join(FONTS_DIR, "IBMPlexSerif-Regular.ttf"),
  path.join(FONTS_DIR, "IBMPlexSerif-Bold.ttf"),
  path.join(FONTS_DIR, "IBMPlexMono-Regular.ttf"),
];

// IBM Plex first (guaranteed present via fontFiles above); the old
// Georgia/Courier New names are kept only as harmless dead weight in
// case this ever runs somewhere system fonts also happen to exist.
const SERIF_STACK = "'IBM Plex Serif', Georgia, 'Times New Roman', serif";
const MONO_STACK = "'IBM Plex Mono', 'Courier New', monospace";
...
function buildUnresolvedCardSvg(params) {
  return classicUnresolved(params, pickTheme(params.editionNumber));
}

function buildResolvedCardSvg(params) {
  return classicResolved(params, pickTheme(params.editionNumber));
}

/**
 * Rasterizes an SVG string to a PNG buffer via resvg-js, at the card's
 * native 640x900 resolution (no fitTo override — renders at the SVG's
 * own width/height). loadSystemFonts is off on purpose: CARD_FONT_FILES
 * above is the complete, sufficient font set for every string this card
 * ever draws, so there's no reason to touch (or depend on) the host's
 * fontconfig at all.
 */
async function rasterizeToPng(svgString) {
...
/**
 * Public API — the only functions anything outside this file should call.
 * Both return a real PNG Buffer, ready to hand straight to
 * uploadImageBytes() the same way a Gemini response would be.
 */
export async function buildUnresolvedCardPng(params) {
  return rasterizeToPng(buildUnresolvedCardSvg(params));
}

export async function buildResolvedCardPng(params) {
  return rasterizeToPng(buildResolvedCardSvg(params));
}
...
<svg width="640" height="900" viewBox="0 0 640 900" xmlns="http://www.w3.org/2000/svg">
  ...
  <text x="320" y="118" font-family="${MONO_STACK}" font-size="13" letter-spacing="6" fill="${MUTED}" text-anchor="middle">MOMENTMARKET</text>
  <text x="320" y="140" font-family="${MONO_STACK}" font-size="11" letter-spacing="4" fill="${THEME_LABEL_TINT[theme]}" text-anchor="middle">UNRESOLVED PREDICTION</text>

  ${goldRule(64, 170, 512, metal, theme)}

  <text x="320" y="230" font-family="${SERIF_STACK}" font-size="30" font-weight="bold" fill="${WHITE}" text-anchor="middle">${escapeXml(match)}</text>

  <text x="320" y="330" font-family="${MONO_STACK}" font-size="12" letter-spacing="3" fill="${MUTED}" text-anchor="middle">PREDICTED EVENT</text>
  <text x="320" y="395" font-family="${SERIF_STACK}" font-size="54" font-weight="bold" fill="${metal}" text-anchor="middle" letter-spacing="2">${escapeXml(eventLabel)}</text>
...
<svg width="640" height="900" viewBox="0 0 640 900" xmlns="http://www.w3.org/2000/svg">
  ...
  <text x="320" y="118" font-family="${MONO_STACK}" font-size="13" letter-spacing="6" fill="${MUTED}" text-anchor="middle">MOMENTMARKET</text>
  <text x="320" y="140" font-family="${MONO_STACK}" font-size="11" letter-spacing="4" fill="${hit ? THEME_LABEL_TINT[theme] : MUTED}" text-anchor="middle">RESOLVED MOMENT</text>

  ${goldRule(64, 170, 512, accentStroke, hit ? theme : null)}

  <text x="320" y="225" font-family="${SERIF_STACK}" font-size="26" font-weight="bold" fill="${WHITE}" text-anchor="middle">${escapeXml(match)}</text>

  <text x="320" y="315" font-family="${MONO_STACK}" font-size="12" letter-spacing="3" fill="${MUTED}" text-anchor="middle">RESULT</text>
  <text x="320" y="${resultLabel.length > 10 ? 380 : 400}" font-family="${SERIF_STACK}" font-size="${resultLabel.length > 10 ? 42 : 56}" font-weight="bold" fill="${resultFill}" text-anchor="middle" letter-spacing="2">${escapeXml(resultLabel)}</text>
  ...
  <text x="320" y="700" font-family="${MONO_STACK}" font-size="10" letter-spacing="1.5" fill="${MUTED}" text-anchor="middle">${onChainVerified ? "ON-CHAIN VERIFIED" : "FEED-CONFIRMED (NO ON-CHAIN STAT KEY)"}</text>
...
function statRow(x, y, label, value) {
  return `
  <text x="${x}" y="${y - 22}" font-family="${MONO_STACK}" font-size="11" letter-spacing="2.5" fill="${MUTED}">${escapeXml(label)}</text>
  <text x="${x}" y="${y + 10}" font-family="${SERIF_STACK}" font-size="22" fill="${WHITE}">${escapeXml(String(value))}</text>`;
}
...
function statusBadge(cx, y, text, positive, theme) {
  ...
  <text x="${cx}" y="${y + 2}" font-family="${MONO_STACK}" font-size="13" letter-spacing="2" fill="${color}" text-anchor="middle">${escapeXml(text)}</text>`;
}

function editionStamp(editionNumber, editionCap, theme) {
  if (editionNumber == null || editionCap == null) return "";
  return `
  <text x="576" y="860" font-family="${MONO_STACK}" font-size="12" letter-spacing="1" fill="${THEME_LABEL_TINT[theme]}" text-anchor="end">No. ${editionNumber} / ${editionCap}</text>`;
}

function footer(text) {
  return `<text x="64" y="860" font-family="${MONO_STACK}" font-size="10" fill="${MUTED}">${escapeXml(text)}</text>`;
}
