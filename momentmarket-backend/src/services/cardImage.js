 ...
 * and `buildResolvedCardPng` below rasterize it to a real PNG buffer via
 * `sharp` before returning it — most wallets (Phantom included) don't
 * render image/svg+xml at all, so the fallback has to be a real raster
 * image to be "functional at the mint level" the same way the Gemini
 * path is. Nothing outside this file should ever see or upload the SVG
 * itself; only buildUnresolvedCardPng/buildResolvedCardPng are meant to
...
/**
 * Fonts are embedded directly in the SVG as base64 @font-face data URIs
 * rather than referenced by name (e.g. "Georgia", "Courier New"). Those
 * system fonts don't exist on a bare Railway/Render/Fly/Coolify Node
 * container — there's no fontconfig, no font packages, nothing — so
 * librsvg's text layout had no glyphs to fall back to and silently
 * rendered every piece of text as a missing-glyph box (□). Embedding the
 * actual font bytes makes this work identically on any host, with zero
 * dependency on what's installed on the server. IBM Plex Serif/Mono are
 * OFL-licensed (see assets/fonts/OFL-IBMPlex.txt) — safe to bundle.
 */
function loadFontBase64(filename) {
  return readFileSync(path.join(FONTS_DIR, filename)).toString("base64");
}

const SERIF_REGULAR_B64 = loadFontBase64("IBMPlexSerif-Regular.ttf");
const SERIF_BOLD_B64 = loadFontBase64("IBMPlexSerif-Bold.ttf");
const MONO_REGULAR_B64 = loadFontBase64("IBMPlexMono-Regular.ttf");
...
const FONT_FACE_STYLE = `
  <style>
    @font-face {
      font-family: 'MMSerif';
      font-weight: 400;
      src: url(data:font/ttf;base64,${SERIF_REGULAR_B64}) format('truetype');
    }
    @font-face {
      font-family: 'MMSerif';
      font-weight: 700;
      src: url(data:font/ttf;base64,${SERIF_BOLD_B64}) format('truetype');
    }
    @font-face {
      font-family: 'MMMono';
      font-weight: 400;
      src: url(data:font/ttf;base64,${MONO_REGULAR_B64}) format('truetype');
    }
  </style>`;

// Embedded font first, then the original stacks as a harmless fallback
// on any host that DOES happen to have system fonts installed.
const SERIF_STACK = "'MMSerif', Georgia, 'Times New Roman', serif";
const MONO_STACK = "'MMMono', 'Courier New', monospace";
...
/**
 * Rasterizes an SVG string to a PNG buffer at the card's native 640x900
 * resolution. `density` is left at sharp's default (72) since the SVG's
 * width/height/viewBox already specify the exact pixel dimensions we want.
 */
