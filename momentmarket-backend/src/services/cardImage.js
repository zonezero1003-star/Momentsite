...
export function buildResolvedCardSvg({ match, actualMinute, outcome }) {
  const hit = outcome === "hit";
  const accent = hit ? GOLD : "#6B7A88";
  const label = hit ? "RESOLVED · HIT" : "RESOLVED · MISS";
...
<svg width="600" height="800" viewBox="0 0 600 800" xmlns="http://www.w3.org/2000/svg">
  ...
  <text x="300" y="420" font-family="'Arial Black', sans-serif" font-size="90" font-weight="900" fill="${accent}" text-anchor="middle" letter-spacing="4">${hit ? "GOAL" : "MISS"}</text>
  ...
  <rect x="40" y="700" width="230" height="44" rx="22" fill="${accent}"/>
  <text x="155" y="728" font-family="monospace" font-size="14" letter-spacing="1.5" fill="${NAVY}" text-anchor="middle" font-weight="bold">${label}</text>
